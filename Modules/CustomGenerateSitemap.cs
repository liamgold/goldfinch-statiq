...
namespace Goldfinch.Modules
{
    ...
    public class CustomGenerateSitemap : Module
    {
        ...
        public CustomGenerateSitemap(Func<string, string> locationFormatter = null)
        ...
        public CustomGenerateSitemap(string sitemapItemOrLocationMetadataKey, Func<string, string> locationFormatter = null)
        ...
        /// <param name="locationFormatter">A location formatter that will be applied to the location of each input after
        /// getting the value of the specified metadata key.</param>
        public CustomGenerateSitemap(Config<object> sitemapItemOrLocation, Func<string, string> locationFormatter = null)
        {
        ...
        private async Task<(string FormattedLocation, SitemapItem Item)> GetSitemapItemAsync(IDocument input, IExecutionContext context)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sitemapItem?.Location))
            {
                ...
                // Apply a trailing slash if we are in a directory (any page that is index.html)
                if (!sitemapItem.Location.Equals("index.html") && sitemapItem.Location.EndsWith("index.html"))
                {
                    location += "/";
                }
            }
